...
freq_X11 <- calc_freq(DATA_CR$X11, "X11")
...
Se compran lechones, con analíticas en el origen. El vehículo de transporte llega limpio. La mayor parte de granjas tiene entregas entre 3 y 6 veces al año.
...
estilizar_tabla(freq_table_X20X23)
...
# Mostrar la tabla
tabla_contingencia_estilizada(DATA_CR, "X26", "X27", dictionary1)
...
# Mostrar la tabla
tabla_contingencia_estilizada(DATA_CR, "X30", "X31", dictionary1)
...
tabla_contingencia_estilizada(DATA_CR, "X35", "X36", dictionary1)
```

La mitad de las granjas no limpia y desinfecta y no mantiene el almacenamiento libre de animales salvajes, perros y gatos, puesto que el vallado está en mal estado.
...
# Mostrar la tabla con las frecuencias y los porcentajes
tabla_contingencia_estilizada(DATA_CR, "X39", "X40", dictionary1)

```

En la mayoría de granjas el proveedor de alimentos puede llenar los silos sin entrar en la zona limpia. En el 35% de granjas no se llenan los silos sin entrar en la zona limpia, y el transportista tiene acceso a las naves.
...
# Mostrar la tabla con las frecuencias y los porcentajes
tabla_contingencia_estilizada(DATA_CR, "X44", "X45", dictionary1)
...
# Mostrar la tabla con las frecuencias y los porcentajes
tabla_contingencia_estilizada(DATA_CR, "X65", "X66", dictionary1)

```

En dos granjas se han detectado jabalíes y no hay vallado perimetral (o está en mal estado).
...
# Mostrar la tabla con las frecuencias y los porcentajes
tabla_contingencia_estilizada(DATA_CR, "X69", "X70", dictionary1)
```
...
# Mostrar la tabla con la descripción encima de cada pregunta
estilizar_tabla(freq_table_X85X90)
...
El número de animales medio después de los cuales se cambia la aguja es 152 animales.
